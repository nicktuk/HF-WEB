    ...
    <main className="min-h-screen" style={{ backgroundColor: '#f7f4ef' }}>
      ...
      <section className="header-texture text-white py-16 sm:py-20 px-4">
        <div className="max-w-3xl mx-auto text-center">
          <span className="text-4xl font-black tracking-[0.16em] text-white">HE·FA</span>
          <p className="text-lg sm:text-xl uppercase tracking-[0.35em] font-extrabold text-white mt-2 mb-5">
            Portal Comercios
          </p>
          <h1 className="text-4xl sm:text-5xl font-bold mb-3">Precios de mayorista, sin vueltas</h1>
          <p className="text-xl text-blue-100/90 max-w-xl mx-auto">
            Para revendedores y comercios de bazar, hogar y electrodomésticos en zona sur GBA.
          </p>
          <p className="text-2xl sm:text-3xl font-extrabold mt-3" style={{ color: '#FF7A50' }}>
        ...
        <div className="max-w-4xl mx-auto mt-10 grid sm:grid-cols-3 gap-4">
          {BENEFICIOS.map(({ icon: Icon, title, text }) => (
            <div key={title} className="bg-white/10 border border-white/15 rounded-2xl p-4 text-center sm:text-left">
              ...
              <p className="text-sm text-blue-100/70 mt-0.5">{text}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="max-w-md mx-auto px-4 -mt-8 pb-16 relative z-10">
        <div className="bg-white rounded-2xl shadow-lg border border-zinc-200/80 p-8">
          <h2 className="text-2xl font-semibold text-zinc-800 mb-6">Iniciar sesión</h2>

          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              <label htmlFor="usuario" className="block text-base font-medium text-zinc-700 mb-1">
                Usuario
              </label>
              <input
                id="usuario"
            ...
            <div className="text-center">
              <Link href="/comercios/olvide-password" className="text-sm text-zinc-500 hover:text-primary-600 hover:underline">
          ...
          <div className="mt-6 pt-6 border-t border-zinc-100 text-center">
            <p className="text-base text-zinc-500 mb-3">¿Todavía no tenés cuenta?</p>
            <Link
              href="/comercios/solicitud"
              className="inline-block bg-white border border-zinc-300 text-zinc-700 rounded-lg px-4 py-2 text-base font-medium hover:bg-zinc-50 transition-colors"
            >
              Solicitá tu código de acceso
            </Link>
          </div>
          <div className="mt-4 text-center">
            <Link href="/comercios/catalogo" className="text-sm text-zinc-400 hover:text-primary-600 hover:underline">
