...
function ResetPasswordForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(true)
    try {
      const res = await fetch('/api/vendedores/auth/reset-password', {
        method: 'POST',
  ...
  if (!token) {
    return (
      <p className="text-sm text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-3">
        ...
        <Link href="/vendedores/olvide-password" className="underline">Pedí uno nuevo</Link>.
      </p>
    )
  }
      ...
      {error && (
        <div className="text-sm text-red-600 bg-red-50 border border-red-200 rounded-lg px-3 py-2">
          ...
          {error.includes('expiró') && (
            <>
              {' '}
              <Link href="/vendedores/olvide-password" className="underline">Pedí un link nuevo</Link>.
            </>
          )}
        </div>
      )}
...
export default function ResetPasswordVendedorPage() {
  return (
