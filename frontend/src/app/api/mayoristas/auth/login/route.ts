...
import { signMayoristaToken, MAYORISTA_COOKIE_NAME } from '@/lib/mayorista-jwt'
...
export async function POST(request: NextRequest) {
  ...
  let res: Response
  try {
    res = await fetch(`${API}/public/mayoristas/login`, {
      method: 'POST',
  ...
  const mayorista = await res.json()

  resetRateLimit(`login:ip:${ip}`)
  resetRateLimit(`login:user:${usuario}`)

  const token = await signMayoristaToken({ mayorista_id: mayorista.id, estado: mayorista.estado })

  const response = NextResponse.json({ ok: true, mayorista })
  response.cookies.set(MAYORISTA_COOKIE_NAME, token, {
    httpOnly: true,
