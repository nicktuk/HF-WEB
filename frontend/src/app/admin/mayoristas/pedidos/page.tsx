...
export default function PedidosMayoristaAdminPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    const res = await apiFetch(`/admin/mayoristas/pedidos?${params}&limit=100`, apiKey)
    if (res.ok) {
  ...
  async function toggleDetalle(id: number) {
    ...
    setExpandedId(id)
    const res = await apiFetch(`/admin/mayoristas/pedidos/${id}`, apiKey)
    if (res.ok) setDetalle(await res.json())
  }

  async function cambiarEstado(id: number, estado: string) {
    setUpdatingId(id)
    await apiFetch(`/admin/mayoristas/pedidos/${id}/estado`, apiKey, {
      method: 'PATCH',
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Pedidos mayoristas</h1>
                  ...
                  <>
                    <tr key={p.id} className={`hover:bg-gray-50 cursor-pointer ${isExpanded ? 'bg-gray-50' : ''}`}>
                      ...
                      <td className="px-4 py-3" onClick={() => toggleDetalle(p.id)}>
                        <p className="font-medium text-gray-900">{p.mayorista_nombre ?? '—'}</p>
                        <p className="text-xs text-gray-400">{p.mayorista_local ?? ''}</p>
                      </td>
