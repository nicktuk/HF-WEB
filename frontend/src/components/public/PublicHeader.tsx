      ...
      <div className={`sticky z-[39] ${isStaging ? 'top-[128px]' : 'top-[104px]'}`} style={{ backgroundColor: '#162844', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
        <div className="container mx-auto px-4">
          ...
          {mobileMenuOpen && orderedCategories.length > 0 && (
            <div className="bg-white border border-zinc-200 rounded-2xl shadow-xl p-4 space-y-1 mb-2 md:hidden">
              {mobileMenuMode === 'categories' ? (
                <>
                  {/* Sort section */}
                  <div className="border-b border-zinc-100 pb-3 mb-1">
                    <p className="text-[11px] font-semibold text-zinc-400 uppercase tracking-widest mb-2 px-3">Ordenar</p>
                    {[
