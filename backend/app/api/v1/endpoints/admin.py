...
async def get_products_admin(
    ...
    without_category: Optional[bool] = Query(default=None),
    ...
    for p in products:
        ...
        admin_products.append(ProductAdminResponse(
            ...
            publish_without_stock=p.publish_without_stock,
...
@router.get(
    "/stats/purchases-by-payer",
    dependencies=[Depends(verify_admin)]
)
async def get_purchases_by_payer(
    request: Request,
    service: ProductService = Depends(get_product_service),
):
    """Get total purchases grouped by payer for dashboard."""
    return service.get_purchases_by_payer()
...
async def get_product_admin(
    ...
    return ProductAdminResponse(
        ...
        is_published=p.is_published,
        publish_without_stock=p.publish_without_stock,
        installments_3=p.installments_3,
        ...
        images=[{
            ...
            "is_primary": img.is_primary,
            "color": img.color,
        } for img in sorted(p.images, key=lambda x: (not x.is_primary, x.display_order))],
        created_at=p.created_at,
        updated_at=p.updated_at,
        source_website_id=p.source_website_id,
        source_website_name=p.source_website.display_name if p.source_website else None,
...
async def update_sale(
    ...
    """Update sale data using item-level delivery/payment checks."""
